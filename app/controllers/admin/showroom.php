...
class Showroom extends CI_Controller {
	...
	public function postshowroom($do = '', $noteid = 0)
	{
		global $admin;
        $data = $this->data;
		...
		if($do == 'post')
        {
            ...
            if ($this->form_validation->run() !== FALSE)
            {
                ...
                $this->note_model->construct( array('noteid' => $noteid, 'title' => $title, 'classid_array' => $classid_array, 'content' => $content, 'prioritynum' => $prioritynum) );
		...
		else
		{
            ...
            $this->class_list_model->db_construct( array('db_where' => array('uid' => $data['user']['uid']) ) );
    ...
    public function delete_showroom($noteid = 0, $hash = '')
    {
        //CSRF過濾
